...
- 影响追踪与召回：上游母样确认污染后，递归追踪全部分装后代，按对象状态分别冻结在库样品、向在借样品借用人发送去重召回通知、把已消耗/已销毁记录列入不可撤销影响清单；处置支持重试且不重复通知，谱系拓扑签名用于检测执行期间新增分装并自动重新计算；解除控制须先记录调查结论，再经两名独立审批人（申请人之外）批准，最终报告说明每个对象的纳入/排除理由。
